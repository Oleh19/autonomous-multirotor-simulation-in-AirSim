...
def build_visual_servo_config(
    ...
    return VisualServoConfig(
        ...
        max_lateral_velocity_m_s=float(
            control_settings.get("servo_max_lateral_velocity_m_s", 0.5)
        ),
        max_vertical_velocity_m_s=float(
            control_settings.get("servo_max_vertical_velocity_m_s", 0.4)
        ),
        max_yaw_rate_deg_s=float(control_settings.get("servo_max_yaw_rate_deg_s", 10.0)),
        yaw_error_deadband_px=float(
            control_settings.get("servo_yaw_error_deadband_px", 10.0)
        ),
        vertical_error_deadband_px=float(
            control_settings.get("servo_vertical_error_deadband_px", 12.0)
        ),
...
def build_visual_servo_controller(
    control_settings: Mapping[str, Any],
    logger: logging.Logger | None = None,
) -> VisualServoController:
    return VisualServoController(
        config=build_visual_servo_config(control_settings),
        logger=logger,
    )
...
def run_visual_servo_step() -> int:
    ...
    servo = build_visual_servo_controller(control_settings, logger=logger)
